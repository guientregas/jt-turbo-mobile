...
function bindFastRouteActions(){
 ...
 ids.forEach(function(id){
  ...
  if(b.__fastBound)return;
  b.__fastBound=true;
  b.onclick=function(){
   if(routeBusy())return;
   var o=A.current&&A.current();
   if(id==='routeGo'&&o){setRouteBusy(true);try{A.navigate(o.id)}finally{setTimeout(function(){setRouteBusy(false)},350)}}
   else if(id==='routeCall'&&o){setRouteBusy(true);try{A.call(o)}finally{setTimeout(function(){setRouteBusy(false)},350)}}
   else if(id==='routeWa'&&o){setRouteBusy(true);try{A.whats(o)}finally{setTimeout(function(){setRouteBusy(false)},900)}}
   else if(id==='routeArrive'){setRouteBusy(true);try{A.arrive()}finally{setTimeout(function(){setRouteBusy(false)},180)}}
   else if(id==='routeDone'&&o){setRouteBusy(true);try{A.finish(true)}finally{setTimeout(function(){setRouteBusy(false)},500)}}
   else if(id==='routeFail'&&o){setRouteBusy(true);try{A.finish(false)}finally{setTimeout(function(){setRouteBusy(false)},350)}}
   else if(id==='routeFinish'){setRouteBusy(true);try{window.__jtFinishRoute&&window.__jtFinishRoute()}finally{setTimeout(function(){setRouteBusy(false)},350)}}
  };
 });
}function(){
'use strict';
