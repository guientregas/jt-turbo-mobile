(function(){
  ...
  function decorate(){document.querySelectorAll('.orderActions button:nth-child(4)').forEach(b=>{if((b.getAttribute('onclick')||'').startsWith('complete(')){b.textContent='✓ Finalizar';b.className='g'}})}
  ...
  window.startBusinessPolling=function(){sync();poll();decorate();setInterval(poll,8000);setInterval(sync,20000);setInterval(decorate,1500)};
  setTimeout(()=>{decorate();if(api())startBusinessPolling()},2500);
})();